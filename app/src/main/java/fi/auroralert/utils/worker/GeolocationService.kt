...
class GeolocationService(private val context: Context): Service(), LocationListener {
    ...
    private fun doWork() {
        ...
        if(gps || net) {
            ...
            if(locationManager != null) {
                ...
                Log.d(TAG, "Gotcha lat: ${loc?.latitude} and lon: ${loc?.longitude}")
                //TODO: update db
                return
            }

            Log.d(TAG, "How can it be null, really?")
            return
        }

        Log.d(TAG, "Failed location?")
    }
